...
import { getAllBooking_Guest, updateBookingStatus_Guest, deleteBooking_Guest } from '../../actions/index'

import '../../css/managebookings.css'
import AddBookingModal from './AddBookingModal';
import EditBookingModal from './EditBookingModal';

class ManageBookingGuest extends Component {

    state = {
        allBooking : [],
        editBooking : [],
        loading : false,
        editBookingType : 'guest'
    }
    ...
    getBookingData = async () => {
        ...
        const resdata = await this.props.getAllBooking_Guest()
        console.log(resdata)
    ...
    setStatus = async (id, defaultStatus) => {
        ...
        if (status) {
            const resdata = await this.props.updateBookingStatus_Guest(id, status)
            console.log(resdata)
    ...
    deleteButton = async (id) => {
        Swal.fire({
            ...
            const res = await this.props.deleteBooking_Guest(id)
            console.log(res)
                if ( res.affectedRows ) {
    ...
    renderBookingByLocation = (inputByAddress) => {
        ...
        let render = arrayBooking.map( booking => {
            ...
            let { booking_status, id, booking_date_formatted, guest, contact_number, car_brand, car_name, car_size, location_type, location_address, booking_price, car_color, car_year, name  } = booking
        ...
        return (
            <div className="ml-5 mr-5">
                <center>
                    ...
                    <AddBookingModal />
                    <EditBookingModal editBooking = {this.state.editBooking} editBookingType = {this.state.editBookingType}  />
...
export default connect(mapStateToProps, { getAllBooking_Guest, updateBookingStatus_Guest, deleteBooking_Guest })(ManageBookingGuest)
